# AI-Quant TODO

## 进行中 🚧

| 任务 | 负责人 | 状态 | 备注 |
|------|--------|------|------|
| - | - | - | - |

## 待办 📋

### 前端
- [ ] 创建 API 服务层（axios 封装，统一请求后端）— 小猪
- [ ] Dashboard 仪表盘对接后端实时数据 — 小猪
- [ ] StockList 股票列表对接 `/api/stocks` + `/api/quotes/realtime` — 小猪
- [ ] BacktestConfig 回测配置对接 `/api/backtest` — 小猪
- [ ] Analysis 收益分析对接回测结果 API — 小猪
- [ ] K线图组件对接 `/api/stocks/{symbol}/history` — 小猪

### 后端
- [ ] 完善回测引擎（目前 `/api/backtest` 只是占位）— minimax
- [ ] 添加后端单元测试（pytest）— minimax
- [ ] 回测结果存储和查询 — minimax

### CI/CD & 部署
- [ ] 配置 GitHub Secrets（SERVER_HOST, SERVER_USER, SERVER_SSH_KEY）— 需要 tq 协助
- [ ] 后端 pytest 集成到 CI — 小猪
- [ ] 前端测试集成到 CI — 小猪
- [ ] nginx 反向代理配置 — 小猪
- [ ] pm2 进程管理配置 — 小猪

### 其他
- [ ] README 更新（完整项目说明）— 待定

## 已完成 ✅

- [x] 后端 API 基础框架（股票列表、历史K线、实时行情）— minimax
- [x] 前端页面框架（Dashboard、StockList、BacktestConfig、Analysis）— minimax
- [x] CI/CD GitHub Actions 基础配置 — minimax
- [x] 小猪服务器环境搭建（Python、Node、pnpm、pm2）— 小猪
- [x] 前后端 build 验证通过 — 小猪

## 协作规则

- 飞书：发布任务、汇报、总结
- GitHub：实际分工，通过此 TODO.md 协调
- 各自通过 heartbeat 定时 git pull 更新任务状态
- 遇到阻塞或需要人类决策 → 飞书找 tq
